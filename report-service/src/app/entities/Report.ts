...
/**
 * Sem @OneToMany / @OneToOne aqui — eles criavam dependência circular com
 * ExportJob e ReportSnapshot no ESM (ReferenceError: cannot access before init).
 * As relações inversas existem nas entidades filhas; buscamos via repository
 * com `relations: ['snapshot']` quando necessário.
 */
@Entity('reports')
export class Report {
  @PrimaryGeneratedColumn()
