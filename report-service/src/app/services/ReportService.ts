...
function reportToResponse(report: Report, withData = false) {
  const base = {
    id: report.id,
    type: report.type,
  ...
  if (withData) {
    return { ...base, data: report.snapshot?.data ?? null };
  }
  return base;
}
...
async function consolidateIndividual(report: Report, token: string): Promise<object> {
  ...
  const responseHistory = (Array.isArray(responses) ? responses : responses.data ?? []).map((r: any) => {
    const categoryResults = (r.categoryResults ?? []).map((cr: any) => {
      if (cr.competencyId && !competencyMap[cr.competencyId]) {
        competencyMap[cr.competencyId] = {
          name: cr.competencyName ?? '',
          latestLevelId: cr.levelId,
          latestLevelName: cr.levelName ?? '',
      ...
      return {
        ...
        categoryName: cr.categoryName ?? null,
    ...
    return {
      ...
      assessmentTitle: r.assessmentTitle ?? null,
  ...
  const competencySummaries = Object.entries(competencyMap).map(([compId, info]) => ({
    ...
    latestLevelId: info.latestLevelId,
    latestLevelName: info.latestLevelName,
    assessmentsCompleted: info.assessmentsCompleted,
  }));

  return {
    userId,
    totalAssessmentsCompleted: responseHistory.length,
    competencySummaries,
    responseHistory,
  };
}
...
async function consolidateGroup(report: Report, token: string): Promise<object> {
  ...
  for (const member of memberList) {
    ...
    for (const r of respList) {
      for (const cr of r.categoryResults ?? []) {
        memberCompetencies[cr.competencyId] = {
          userId: member.id,
          competencyId: cr.competencyId,
          competencyName: cr.competencyName ?? null,
          latestLevelId: cr.levelId,
          latestLevelName: cr.levelName ?? null,
          assessmentsCompleted: (memberCompetencies[cr.competencyId]?.assessmentsCompleted ?? 0) + 1,
        };

        if (!competencyAverages[cr.competencyId]) {
          competencyAverages[cr.competencyId] = { name: cr.competencyName ?? '', total: 0, count: 0 };
        }
  ...
  return {
    ...
    competencyAverages: Object.entries(competencyAverages).map(([id, info]) => ({
      competencyId: Number(id),
      competencyName: info.name,
      averageLevelId: Math.round(info.total / info.count),
      averageScore: Math.round((info.total / info.count) * 10) / 10,
    })),
    memberBreakdown,
  };
}
...
async function consolidateCompetency(report: Report, token: string): Promise<object> {
  ...
  for (const comp of competencyList) {
    if (report.filterCompetencyId && comp.id !== report.filterCompetencyId) continue;

    distributions.push({
      competencyId: comp.id,
      competencyName: comp.name ?? null,
      totalRespondents: 0,
      levelDistribution: [],
    });
  }
  ...
  return {
    totalRespondents: 0,
    competencyDistributions: distributions,
  };
}
...
export async function createReport(
  ...
  // userId no filtro: se não for ADMIN, força o próprio ID
  if (!isAdmin) {
    report.filterUserId = null;
  }
...
export async function processReport(reportId: number, token: string): Promise<void> {
  ...
  try {
    ...
    switch (report.type) {
      case 'INDIVIDUAL':
        data = await consolidateIndividual(report, token);
        break;
      case 'GROUP':
        data = await consolidateGroup(report, token);
        break;
      case 'COMPETENCY':
        data = await consolidateCompetency(report, token);
        break;
      case 'ASSESSMENT':
        data = await consolidateAssessment(report, token);
        break;
      default:
    ...
    console.error(`[ReportService] Falha ao processar relatório ${reportId}:`, err);
...
export async function listReports(
  ...
  if (!isAdmin) {
    qb.where('r.requested_by_user_id = :userId', { userId });
  }
...
export async function listMyReports(userId: number, status?: string) {
  const where: FindOptionsWhere<Report> = { requestedByUserId: userId, type: 'INDIVIDUAL' };
...
export async function getReport(reportId: number, userId: number, isAdmin: boolean) {
  ...
  return reportToResponse(report, true);
}
...
export async function getMyProgress(
  ...
  for (const r of respList.sort((a: any, b: any) =>
    ...
    for (const cr of r.categoryResults ?? []) {
      ...
      progressMap[cr.competencyId].entries.push({
        responseId: r.id,
        assessmentId: r.assessmentId,
        levelId: cr.levelId,
        levelName: cr.levelName ?? null,
        achievedAt: r.submittedAt,
      });
    }
  }
